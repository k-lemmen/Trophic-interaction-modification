...
t_start <- Sys.time()
# Run Grid #
CP_t2_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2_flexLV_pe <- foreach(i = 1:nrow(t2.sim_grid),
                          .combine = 'rbind',
...
CP_t2_flexLV_pe %>% arrange(BIC)

#save(CP_t2_flexLV_pe, file = "CP_t2_flexLV_pe.RData")

###### Best Model #####
(CP_T2_PECs <- CP_t2_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min

min_T2 <- CP_T2_PECs %>% filter(BIC == min(CP_T2_PECs$BIC, na.rm = T)) %>% select(c(a_log.est:sigma2.est, BIC))

if(nrow(min_T2)>1){
  min_T2 <- CP_T2_PECs %>% filter(strtpt %in% sample(filter(CP_T2_PECs, BIC <= min(CP_T2_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_T2_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.a1.h1.lv,
                            method= "Nelder-Mead",
                            control = list(maxit = 5000),
...
#save(CP_min_T2_flexLV_mod, file = "CP_T2_flexLV_mod.Rdata")
tidy(CP_min_T2_flexLV_mod)

(nrow(CP_T2_PECs)/nrow(CP_t2_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_T2_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t2.a12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2.a12_flexLV_pe <- foreach(i = 1:nrow(t2.a12.sim_grid),
                              .combine = 'rbind',
                              .options.multicore = mcoptions,
                              .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                              .errorhandling = "pass") %dopar%
...
CP_t2.a12_flexLV_pe %>% arrange(BIC)
#save(CP_t2.a12_flexLV_pe, file = "CP_t2.a12_flexLV_pe.RData")

###### Best Model #####
(CP_t2.a12_PECs <- CP_t2.a12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2.a12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t2.a12)>1){
  min_t2.a12 <- CP_t2.a12_PECs %>% filter(strtpt %in% sample(filter(CP_t2.a12_PECs, BIC <= min(CP_t2.a12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t2.a12_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.a1.h1.lv,
                                method= "Nelder-Mead",
                                control = list(maxit = 5000),
...
#save(CP_min_t2.a12_flexLV_mod, file = "CP_t2.a12_flexLV_mod.Rdata")
tidy(CP_min_t2.a12_flexLV_mod)

(nrow(CP_t2.a12_PECs)/nrow(CP_t2.a12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t2.a12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t2.h12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2.h12_flexLV_pe <- foreach(i = 1:nrow(t2.h12.sim_grid),
                              .combine = 'rbind',
                              .options.multicore = mcoptions,
                              .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                              .errorhandling = "pass") %dopar%
...
CP_t2.h12_flexLV_pe %>% arrange(BIC)

#save(CP_t2.h12_flexLV_pe, file = "CP_t2.h12_flexLV_pe.RData")

###### Best Model #####
(CP_t2.h12_PECs <- CP_t2.h12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2.h12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t2.h12)>1){
  min_t2.h12 <- CP_t2.h12_PECs %>% filter(strtpt %in% sample(filter(CP_t2.h12_PECs, BIC <= min(CP_t2.h12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t2.h12_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.a1.h1.lv,
                                method= "Nelder-Mead",
                                control = list(maxit = 5000),
...
#save(CP_min_t2.h12_flexLV_mod, file = "CP_t2.h12_flexLV_mod.Rdata")
tidy(CP_min_t2.h12_flexLV_mod)

(nrow(CP_t2.h12_PECs)/nrow(CP_t2.h12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t2.h12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t2.a12.h12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2.a12.h12_flexLV_pe <- foreach(i = 1:nrow(t2.a12.h12.sim_grid),
                                  .combine = 'rbind',
                                  .options.multicore = mcoptions,
                                  .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                                  .errorhandling = "pass") %dopar%
...
CP_t2.a12.h12_flexLV_pe %>% arrange(BIC)

#save(CP_t2.a12.h12_flexLV_pe, file = "CP_t2.a12.h12_flexLV_pe.RData")

###### Best Model #####
(CP_t2.a12.h12_PECs <- CP_t2.a12.h12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2.a12.h12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t2.a12.h12)>1){
  min_t2.a12.h12 <- CP_t2.a12.h12_PECs %>% filter(strtpt %in% sample(filter(CP_t2.a12.h12_PECs, BIC <= min(CP_t2.a12.h12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t2.a12.h12_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.a1.h1.lv,
                                    method= "Nelder-Mead",
                                    control = list(maxit = 5000),
...
#save(CP_min_t2.a12.h12_flexLV_mod, file = "CP_t2.a12.h12_flexLV_mod.Rdata")
tidy(CP_min_t2.a12.h12_flexLV_mod)

(nrow(CP_t2.a12.h12_PECs)/nrow(CP_t2.a12.h12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t2.a12.h12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t2.vv_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2.vv_flexLV_pe <- foreach(i = 1:nrow(t2.vv.sim_grid),
                             .combine = 'rbind',
                             .options.multicore = mcoptions,
                             .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                             .errorhandling = "pass") %dopar%
...
CP_t2.vv_flexLV_pe %>% arrange(BIC)

#save(CP_t2.vv_flexLV_pe, file = "CP_t2.vv_flexLV_pe.RData")

###### Best Model #####
(CP_t2.vv_PECs <- CP_t2.vv_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2.vv_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min

set.seed(273) 
min_t2.vv <- CP_t2.vv_PECs %>% filter(BIC == min(CP_t2.vv_PECs$BIC, na.rm = T)) %>% select(c(a_log.est:sigma2.est, BIC))

if(nrow(min_t2.vv)>1){
  min_t2.vv <- CP_t2.vv_PECs %>% filter(strtpt %in% sample(filter(CP_t2.vv_PECs, BIC <= min(CP_t2.vv_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t2.vv_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.h1.VV.lv,
                               method= "Nelder-Mead",
                               control = list(maxit = 5000),
...
#save(CP_min_t2.vv_flexLV_mod, file = "CP_t2.vv_flexLV_mod.Rdata")
tidy(CP_min_t2.vv_flexLV_mod)

(nrow(CP_t2.vv_PECs)/nrow(CP_t2.vv_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t2.vv_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t2.vv.h12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2.vv.h12_flexLV_pe <- foreach(i = 1:nrow(t2.vv.h12.sim_grid),
                                 .combine = 'rbind',
                                 .options.multicore = mcoptions,
                                 .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                                 .errorhandling = "pass") %dopar%
...
CP_t2.vv.h12_flexLV_pe %>% arrange(BIC)

#save(CP_t2.vv.h12_flexLV_pe, file = "CP_t2.vv.h12_flexLV_pe.RData")

###### Best Model #####
(CP_t2.vv.h12_PECs <- CP_t2.vv.h12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2.vv.h12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t2.vv.h12)>1){
  min_t2.vv.h12 <- CP_t2.vv.h12_PECs %>% filter(strtpt %in% sample(filter(CP_t2.vv.h12_PECs, BIC <= min(CP_t2.vv.h12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t2.vv.h12_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.h1.VV.lv,
                                   method= "Nelder-Mead",
                                   control = list(maxit = 5000),
...
#save(CP_min_t2.vv.h12_flexLV_mod, file = "CP_t2.vv.h12_flexLV_mod.Rdata")
tidy(CP_min_t2.vv.h12_flexLV_mod)

(nrow(CP_t2.vv.h12_PECs)/nrow(CP_t2.vv.h12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t2.vv.h12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t2.cm_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t2.cm_flexLV_pe <- foreach(i = 1:nrow(t2.cm.sim_grid),
                             .combine = 'rbind',
                             .options.multicore = mcoptions,
                             .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                             .errorhandling = "pass") %dopar%
...
CP_t2.cm_flexLV_pe %>% arrange(BIC)

#save(CP_t2.cm_flexLV_pe, file = "CP_t2.cm_flexLV_pe.RData")

###### Best Model #####
(CP_t2.cm_PECs <- CP_t2.cm_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t2.cm_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min

min_t2.cm <- CP_t2.cm_PECs %>% filter(BIC == min(CP_t2.cm_PECs$BIC, na.rm = T)) %>% select(c(a_log.est:sigma2.est, BIC))

if(nrow(min_t2.cm)>1){
  min_t2.cm <- CP_t2.cm_PECs %>% filter(strtpt %in% sample(filter(CP_t2.cm_PECs, BIC <= min(CP_t2.cm_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t2.cm_flexLV_mod =  mle2(minuslogl = nll.odeint.t2.CM.lv,
                               method= "Nelder-Mead",
                               control = list(maxit = 5000),
...
#save(CP_min_t2.cm_flexLV_mod, file = "CP_t2.cm_flexLV_mod.Rdata")
tidy(CP_min_t2.cm_flexLV_mod)

(nrow(CP_t2.cm_PECs)/nrow(CP_t2.cm_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t2.cm_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t3_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t3_flexLV_pe <- foreach(i = 1:nrow(t3.sim_grid),
                          .combine = 'rbind',
                          .options.multicore = mcoptions,
                          .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                          .errorhandling = "pass") %dopar%
...
CP_t3_flexLV_pe %>% arrange(BIC)

#save(CP_t3_flexLV_pe, file = "CP_t3_flexLV_pe.RData")

###### Best Model #####
(CP_t3_PECs <- CP_t3_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t3_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min

min_T3 <- CP_t3_PECs %>% filter(BIC == min(CP_t3_PECs$BIC, na.rm = T)) %>% select(c(a_log.est:sigma2.est, BIC))

if(nrow(min_T3)>1){
  min_T3 <- CP_t3_PECs %>% filter(strtpt %in% sample(filter(CP_t3_PECs, BIC <= min(CP_t3_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t3_flexLV_mod =  mle2(minuslogl = nll.odeint.t3.a1.h1.lv,
                            method= "Nelder-Mead",
                            control = list(maxit = 5000),
...
#save(CP_min_t3_flexLV_mod, file = "CP_t3_flexLV_mod.Rdata")
tidy(CP_min_t3_flexLV_mod)

(nrow(CP_t3_PECs)/nrow(CP_t3_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t3_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t3.a12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t3.a12_flexLV_pe <- foreach(i = 1:nrow(t3.a12.sim_grid),
                              .combine = 'rbind',
                              .options.multicore = mcoptions,
                              .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                              .errorhandling = "pass") %dopar%
...
CP_t3.a12_flexLV_pe %>% arrange(BIC)
#save(CP_t3.a12_flexLV_pe, file = "CP_t3.a12_flexLV_pe.RData")

###### Best Model #####
(CP_t3.a12_PECs <- CP_t3.a12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t3.a12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t3.a12)>1){
  min_t3.a12 <- CP_t3.a12_PECs %>% filter(strtpt %in% sample(filter(CP_t3.a12_PECs, BIC <= min(CP_t3.a12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t3.a12_flexLV_mod =  mle2(minuslogl = nll.odeint.t3.a1.h1.lv,
                                method= "Nelder-Mead",
                                control = list(maxit = 5000),
...
#save(CP_min_t3.a12_flexLV_mod, file = "CP_t3.a12_flexLV_mod.Rdata")
tidy(CP_min_t3.a12_flexLV_mod)

(nrow(CP_t3.a12_PECs)/nrow(CP_t3.a12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t3.a12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t3.h12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t3.h12_flexLV_pe <- foreach(i = 1:nrow(t3.h12.sim_grid),
                              .combine = 'rbind',
                              .options.multicore = mcoptions,
                              .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                              .errorhandling = "pass") %dopar%
...
CP_t3.h12_flexLV_pe %>% arrange(BIC)

#save(CP_t3.h12_flexLV_pe, file = "CP_t3.h12_flexLV_pe.RData")

###### Best Model #####
(CP_t3.h12_PECs <- CP_t3.h12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t3.h12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t3.h12)>1){
  min_t3.h12 <- CP_t3.h12_PECs %>% filter(strtpt %in% sample(filter(CP_t3.h12_PECs, BIC <= min(CP_t3.h12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t3.h12_flexLV_mod =  mle2(minuslogl = nll.odeint.t3.a1.h1.lv,
                                method= "Nelder-Mead",
                                control = list(maxit = 5000),
...
#save(CP_min_t3.h12_flexLV_mod, file = "CP_t3.h12_flexLV_mod.Rdata")
tidy(CP_min_t3.h12_flexLV_mod)

(nrow(CP_t3.h12_PECs)/nrow(CP_t3.h12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t3.h12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t3.a12.h12_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t3.a12.h12_flexLV_pe <- foreach(i = 1:nrow(t3.a12.h12.sim_grid),
                                  .combine = 'rbind',
                                  .options.multicore = mcoptions,
                                  .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                                  .errorhandling = "pass") %dopar%
...
CP_t3.a12.h12_flexLV_pe %>% arrange(BIC)

#save(CP_t3.a12.h12_flexLV_pe, file = "CP_t3.a12.h12_flexLV_pe.RData")

###### Best Model #####
(CP_t3.a12.h12_PECs <- CP_t3.a12.h12_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t3.a12.h12_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min
...
if(nrow(min_t3.a12.h12)>1){
  min_t3.a12.h12 <- CP_t3.a12.h12_PECs %>% filter(strtpt %in% sample(filter(CP_t3.a12.h12_PECs, BIC <= min(CP_t3.a12.h12_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t3.a12.h12_flexLV_mod =  mle2(minuslogl = nll.odeint.t3.a1.h1.lv,
                                    method= "Nelder-Mead",
                                    control = list(maxit = 5000),
...
#save(CP_min_t3.a12.h12_flexLV_mod, file = "CP_t3.a12.h12_flexLV_mod.Rdata")
tidy(CP_min_t3.a12.h12_flexLV_mod)

(nrow(CP_t3.a12.h12_PECs)/nrow(CP_t3.a12.h12_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t3.a12.h12_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t3.vv_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t3.vv_flexLV_pe <- foreach(i = 1:nrow(t3.vv.sim_grid),
                             .combine = 'rbind',
                             .options.multicore = mcoptions,
                             .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                             .errorhandling = "pass") %dopar%
...
CP_t3.vv_flexLV_pe %>% arrange(BIC)

#save(CP_t3.vv_flexLV_pe, file = "CP_t3.vv_flexLV_pe.RData")

###### Best Model #####
(CP_t3.vv_PECs <- CP_t3.vv_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t3.vv_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min

min_t3.vv <- CP_t3.vv_PECs %>% filter(BIC == min(CP_t3.vv_PECs$BIC, na.rm = T)) %>% select(c(a_log.est:sigma2.est, BIC))

if(nrow(min_t3.vv)>1){
  min_t3.vv <- CP_t3.vv_PECs %>% filter(strtpt %in% sample(filter(CP_t3.vv_PECs, BIC <= min(CP_t3.vv_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t3.vv_flexLV_mod =  mle2(minuslogl = nll.odeint.T3.h1.VV.lv,
                               method= "Nelder-Mead",
                               control = list(maxit = 5000),
...
#save(CP_min_t3.vv_flexLV_mod, file = "CP_t3.vv_flexLV_mod.Rdata")
tidy(CP_min_t3.vv_flexLV_mod)

(nrow(CP_t3.vv_PECs)/nrow(CP_t3.vv_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t3.vv_flexLV_mod)
...
t_start <- Sys.time()
# Run Grid #
CP_t3.cm_flexLV_pe <- NA # name of file where parallel runs will be save
mcoptions <- list(preschedule=FALSE)

CP_t3.cm_flexLV_pe <- foreach(i = 1:nrow(t3.cm.sim_grid),
                             .combine = 'rbind',
                             .options.multicore = mcoptions,
                             .packages = c('bbmle', 'odeintr', 'tidyverse', 'R.utils'),
                             .errorhandling = "pass") %dopar%
...
CP_t3.cm_flexLV_pe %>% arrange(BIC)

#save(CP_t3.cm_flexLV_pe, file = "CP_t3.cm_flexLV_pe.RData")

###### Best Model #####
(CP_t3.cm_PECs <- CP_t3.cm_flexLV_pe %>% 
   filter(converge == 0) %>% 
   filter(BIC <= min(CP_t3.cm_flexLV_pe$BIC, na.rm = T)+2)) #check for similarity in parameter estimates within 2 AIC of min

min_t3.cm <- CP_t3.cm_PECs %>% filter(BIC == min(CP_t3.cm_PECs$BIC, na.rm = T)) %>% select(c(a_log.est:sigma2.est, BIC))

if(nrow(min_t3.cm)>1){
  min_t3.cm <- CP_t3.cm_PECs %>% filter(strtpt %in% sample(filter(CP_t3.cm_PECs, BIC <= min(CP_t3.cm_PECs$BIC, na.rm = T)+0.1)$strtpt,1))
}

CP_min_t3.cm_flexLV_mod =  mle2(minuslogl = nll.odeint.T3.CM.lv,
                               method= "Nelder-Mead",
                               control = list(maxit = 5000),
...
#save(CP_min_t3.cm_flexLV_mod, file = "CP_t3.cm_flexLV_mod.Rdata")
tidy(CP_min_t3.cm_flexLV_mod)

(nrow(CP_t3.cm_PECs)/nrow(CP_t3.cm_flexLV_pe %>% filter(converge == 0))) * 100 # percent of starting combinations within 2 AIC that 
BIC(CP_min_t3.cm_flexLV_mod)

t_end <- Sys.time()
mod13_time <- t_end-t_start

#### BIC Table ####
CP_BIC<-as.data.frame(BIC(CP_min_T2_flexLV_mod,
                          CP_min_t2.a12_flexLV_mod,
                          CP_min_t2.h12_flexLV_mod,
                          CP_min_t2.a12.h12_flexLV_mod,
                          CP_min_t2.vv_flexLV_mod,
                          CP_min_t2.vv.h12_flexLV_mod,
                          CP_min_t2.cm_flexLV_mod,
                          CP_min_t3_flexLV_mod,
                          CP_min_t3.a12_flexLV_mod,
                          CP_min_t3.h12_flexLV_mod,
                          CP_min_t3.a12.h12_flexLV_mod,
                          CP_min_t3.vv_flexLV_mod,
                          CP_min_t3.cm_flexLV_mod))
CP_BIC$model<-seq(1, nrow(CP_BIC), by =1); CP_BIC$dBIC<-round(CP_BIC$BIC-BIC(CP_min_t2.a12_flexLV_mod),1)
...
save.image(file = "TIM_fit_flex.RData")

#### Parameter Table
pe_list <- list(coef(CP_min_T2_flexLV_mod),
      coef(CP_min_t2.a12_flexLV_mod),
      coef(CP_min_t2.h12_flexLV_mod),
      coef(CP_min_t2.a12.h12_flexLV_mod),
      coef(CP_min_t2.vv_flexLV_mod),
      coef(CP_min_t2.vv.h12_flexLV_mod),
      coef(CP_min_t2.cm_flexLV_mod),
      coef(CP_min_t3_flexLV_mod),
      coef(CP_min_t3.a12_flexLV_mod),
      coef(CP_min_t3.h12_flexLV_mod),
      coef(CP_min_t3.a12.h12_flexLV_mod),
      coef(CP_min_t3.vv_flexLV_mod),
      coef(CP_min_t3.cm_flexLV_mod)
      )

pe_table <- as.data.frame(do.call(rbind, lapply(lapply(pe_table, unlist), "[",
                      unique(unlist(c(sapply(pe_table,names)))))))
