...
# ----------------------------------------------------------------------------------------------------------------------------------
# Fitting Function for Type II Functional Response with TIM on space clearance rate (VV) and handling time (constant, linear) with Lotka-Voltera competition
# ----------------------------------------------------------------------------------------------------------------------------------
#
...
#
# Linear constant for a allows for the density of the modifier to impact the rate at which the predator clears the environment of prey - 
# linear scaling previously seen in Hauzy et al. 2010, and Mocq et al. 2021 (no code provided) - a' = (a + a12*R2)
#
...
dxdt[0] = x[0] * c * (((a * x[1]) / (kr + x[1]))  * x[1]) / (1 + ((a * x[1]) / (kr + x[1])) * (h + h12 * x[2]) * x[1] + w * x[2]) - x[0] * m;

dxdt[1] = - (((a * x[1]) / (kr + x[1]))  * x[1]) / (1 + ((a * x[1]) / (kr + x[1])) * (h + h12 * x[2]) * x[1] + w * x[2])  * x[0] + r1 * x[1] * (1- alpha11*x[1] - alpha12*x[2]);
...
eq.odeint.T3.h1.VV.lv = function(start.v, a, kr, h, h12, w, r1, alpha11, alpha12, r2, alpha22, alpha21, c, m, Tt, timesteplength){
  FRS_T3_h1_VV_lv_set_params(a=a, kr=kr, h=h, h12=h12, w=w, r1=r1, alpha11=alpha11, alpha12=alpha12, r2=r2, alpha22=alpha22, alpha21=alpha21, c=c, m=m)
  FRS_T3_h1_VV_lv(start.v,Tt,timesteplength)
}

#### Solve model eaten.odeint ####
eaten.odeint.T3.h1.VV.lv = function(N0, a, kr, h, h12, w, r1, alpha11, alpha12, r2, alpha22, alpha21, c, m, Tt, M0, P, steps=100){
  ...
  for(i.eaten in 1:length(N0)){
    ...
    temp = eq.odeint.T3.h1.VV.lv(start.v = c(P[i.eaten], N0[i.eaten], M0[i.eaten]),
                                      a = a,
                                      kr = kr,
                                      h = h,
                                      h12 = h12,
...
#### Determine nll ####
nll.odeint.T3.h1.VV.lv = function(a_log, h_log, h12, kr_log, log_w, r1_log, alpha11, alpha12, r2_log, alpha22, alpha21, c_log, m, N0, Ndead, P, P.end, M0, M0.end, Tt, steps=100, sigma, sigma2, verbose = F){ #log_w
  if(sigma <= 0) return(Inf)
  temp2 = eaten.odeint.T3.h1.VV.lv(N0 = N0,
                                      a = exp(a_log),
                                      kr = exp(kr_log),
                                      h = exp(h_log),
                                      h12 = h12,
