...
# ----------------------------------------------------------------------------------------------------------------------------------
# Fitting Function for Functional response with an asymptotic search clearance rate, Crowley-Martin interference and Lotka-Voltera competition
# ----------------------------------------------------------------------------------------------------------------------------------
#
...
dxdt[0] = x[0] * c * ((((a * x[1]) / (kr + x[1]))  * x[1]) / ((1 + ((a * x[1]) / (kr + x[1])) * h * x[1]) * (1 + w * x[2]))) - x[0] * m;

dxdt[1] = - ((((a * x[1]) / (kr + x[1]))  * x[1]) / ((1 + ((a * x[1]) / (kr + x[1])) * h * x[1]) * (1 + w * x[2])))  * x[0] + r1 * x[1] * (1- alpha11*x[1] - alpha12*x[2]);
...
eq.odeint.T3.CM.lv = function(start.v, a, kr, h, w, r1, alpha11, alpha12, r2, alpha22, alpha21, c, m, Tt, timesteplength){
  FRS_T3_CM_lv_set_params(a=a, kr=kr, h=h, w=w, r1=r1, alpha11=alpha11, alpha12=alpha12, r2=r2, alpha22=alpha22, alpha21=alpha21, c=c, m=m)
  FRS_T3_CM_lv(start.v,Tt,timesteplength)
}


#### Solve model eaten.odeint ####
eaten.odeint.T3.CM.lv = function(N0, a, kr, h, w, r1, alpha11, alpha12, r2, alpha22, alpha21, c, m, Tt, M0, P, steps=100){
  ...
  for(i.eaten in 1:length(N0)){
    ...
    temp = eq.odeint.T3.CM.lv(start.v = c(P[i.eaten], N0[i.eaten], M0[i.eaten]),
                              a = a,
                              kr = kr,
                              h = h,
                              w = w, 
...
#### Determine nll ####
nll.odeint.T3.CM.lv = function(a_log, h_log, kr_log, log_w, r1_log, alpha11, alpha12, r2_log, alpha22, alpha21, c_log, m, N0, Ndead, P, P.end, M0, M0.end, Tt, steps=100, sigma, sigma2, verbose = F){
  if(sigma <= 0) return(Inf)
  temp2 = eaten.odeint.T3.CM.lv(N0 = N0,
                                a = exp(a_log),
                                kr = exp(kr_log),
                                h = exp(h_log),
                                w = exp(log_w),
