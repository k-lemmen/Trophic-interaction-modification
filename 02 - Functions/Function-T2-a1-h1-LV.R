...
# ----------------------------------------------------------------------------------------------------------------------------------
# Fitting Function for Type II Functional Response with TIM on space clearance rate (linear) and handling time (constant) with Lotka-Voltera competition
# ----------------------------------------------------------------------------------------------------------------------------------
#
...
dxdt[0] = x[0] * c * (((a + a12 * x[2]) * x[1]) / (1 + (a + a12 * x[2]) * (h + h12 * x[2]) * x[1])) - x[0] * m;

dxdt[1] = - (((a + a12 * x[2]) * x[1]) / (1 + (a + a12 * x[2]) * (h + h12 * x[2]) * x[1])) * x[0] + r1 * x[1] * (1- alpha11*x[1] - alpha12*x[2]);
...
eq.odeint.t2.a1.h1.lv = function(start.v, a, a12, h, h12, r1, alpha11, alpha12, r2, alpha22, alpha21, c, m, Tt, timesteplength){
  FRS_t2_a1_h1_lv_set_params(a=a, a12=a12, h=h, h12=h12, r1=r1, alpha11=alpha11, alpha12=alpha12, r2=r2, alpha22=alpha22, alpha21=alpha21, c=c, m=m)
  FRS_t2_a1_h1_lv(start.v,Tt,timesteplength)
}


#### Solve model eaten.odeint ####
eaten.odeint.t2.a1.h1.lv = function(N0, a, a12, h, h12, r1, alpha11, alpha12, r2, alpha22, alpha21, c, m, Tt, M0, P, steps=100){
  ...
  for(i.eaten in 1:length(N0)){
    ...
    temp = eq.odeint.t2.a1.h1.lv(start.v = c(P[i.eaten], N0[i.eaten], M0[i.eaten]),
                                    a = a,
                                    a12 = a12,
                                    h = h,
                                    h12 = h12,
...
#### Determine nll ####
nll.odeint.t2.a1.h1.lv = function(a_log, a12, h_log, h12, r1_log, alpha11, alpha12, r2_log, alpha22, alpha21, c_log, m, N0, Ndead, P, P.end, M0, M0.end, Tt, steps=100, sigma, sigma2, verbose = F){ #log_w
  if(sigma <= 0) return(Inf)
  temp2 = eaten.odeint.t2.a1.h1.lv(N0 = N0,
                                      a = exp(a_log),
                                      a12 = a12,
                                      h = exp(h_log),
                                      h12 = h12,
